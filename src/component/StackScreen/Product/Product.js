...
import { Keyboard, StyleSheet, View, Text, Animated, Image, TextInput, Linking, BackHandler, PanResponder, TouchableOpacity, SafeAreaView, Modal, ScrollView, Dimensions, ActivityIndicator, Alert, LogBox, Platform, Button, KeyboardAvoidingView } from 'react-native';
...
export default function Product() {
    ...
    const [wishListId, setWishListId] = useState(false);
    ...
    const getProductList = async () => {
        ...
        console.log("array length", categoryValue.current);
    ...
    const getProductRecords = async () => {
        ...
        axios.post(baseUrl + endPoint.current, obj)
            .then((response) => {
                if (response.data) {
                    console.log("product list", JSON.stringify(product));
                    setPageCount(response.data.totalPages);
                    setRecordCount(response.data.totalCount);
    ...
    const saveCollectionName = async () => {
        if (collectionName != "") {
            setCheckedWishList({});
            if (collectionName.match(/^[a-zA-Z0-9 ]*$/)) {
                setCheckedWishList({});
                const token = await AsyncStorage.getItem("authToken");
                let dataObj = {
                    "id": 0,
                ...
                alert("Only Character and Number.")
            }
        } else {
            alert("Create wishlist first.")
        }
    }
        ...
        if (item.wishlist == true) {
            ...
            Alert.alert(
                ...
                [
                    ...
                    { text: "OK", onPress: () => pressLike(itemId, item) }
                ]
            );
        } else {
            setCheckedWishList({});
    ...
    const pressLike = async (itemIdd, item) => {
        var wishListData = [];
        for (let index = 0; index < wishlistCategory.length; index++) {
            if (wishlistCategory[index].isSelected == true) {
                wishListData.push(wishlistCategory[index].id);
            }
    ...
    const onSavePress = async () => {
        ...
        if (wishListId == true) {
            alert("Select the wishList first.")
        } else {
            var wishListData = [];
            for (let index = 0; index < wishlistCategory.length; index++) {
                if (wishlistCategory[index].isSelected == true) {
            ...
            console.log("product save", dataObj);
            await axios.post(baseUrl + "Product/UpdateProductWishlist", dataObj, {
                headers: {
                    "Content-Type": "application/json"
                }
    ...
    const onDiscard = async (itemId, indexx) => {
        Alert.alert(
            "Confirmation",
            ...
            [
                ...
                { text: "OK", onPress: () => pressDiscard(itemId, indexx) }
            ]
        );
    }

    const pressDiscard = async (itemIdd, indexx) => {
        const filterType = await AsyncStorage.getItem("Ali");
        const token = await AsyncStorage.getItem("authToken");
        ...
        await axios.post(baseUrl + "Product/AddEditProductStatus", dataObj, {
            ...
            setLoader(true);
            let productIndex_ = 0;
            product.forEach((item, index11) => {
                if (item.id == itemIdd) {
                    productIndex_ = index11;
                }
            });
            product.splice(productIndex_, 1);
            getProductRecords();
        });

        // const filterShortList = product.filter((item) => item.id !== itemIdd);
        // product.splice(1, product.length, ...filterShortList);
                                    ...
                                    <SafeAreaView style={{ height: "100%", backgroundColor: "#000000aa" }}>
                                        <View
                                            style={{
                                                ...
                                                padding: "3%",
                                                position: "absolute", bottom: Platform.OS == "android" ? 0.1 : 0
                                            }}>
                                            <View>
                                                ...
                                                <ScrollView style={{ maxHeight: "60%" }} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: "5%" }}>

                                                    {
                                                        wishlistCategory && wishlistCategory.map((item, index) => {
                                                            return (
                                                                <View style={{ display: "flex", flexDirection: "row", alignItems: "center", marginTop: "3%", justifyContent: "space-between", width: "98%" }}>
                                                                    <View style={{ display: "flex", flexDirection: "row", alignItems: "center" }}>
                                                                    ...
                                                                    <CheckBox
                                                                        disabled={false}
                                                                        value={item.isSelected == true ? item.isSelected : checkedWishList[item.id]}
                                                ...
                                                <TouchableOpacity
                                                    onPress={() => onSavePress()}
                                                    style={{ backgroundColor: "#284B46", justifyContent: "center", alignItems: "center", height: 35, width: "100%", alignSelf: "center", marginTop: "4%" }}>
                                                    <Text style={{ fontSize: 15, color: "white", fontFamily: "Poppins-Medium" }}>Save</Text>
                                                </TouchableOpacity>
                                            </View>
                                        </View>
